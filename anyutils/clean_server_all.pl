...
if (!defined($repo32) && !defined($repo64) && !defined($repoARM)) {
	if (defined $repopath) {
		$repo32 = $repopath . $path_separator . 'i586';
		$repo64 = $repopath . $path_separator . 'x86_64';
		$repoARM = $repopath . $path_separator . 'armv5te';
	} else {
		$help = 1;
	}
}
...
if ($help) {
	...
	print "--repoARM=path DEPRECATED BY 'repopth': specify path to ARM repo (default $repoARM).\n";
	...
	print "--dupsARM      identify duplicate packages within the ARM repo\n";
	print "--location    identify packages not residing in same section b/w 32/64 bit & ARM repos\n";
	print "--notexist    identify packages that do not exist in both 32 and 64 bit & ARM repos\n";
	print "--vercheck    identify packages that have different version/revision in both 32/64 bit & ARM repos\n";
...
for my $prefix (($repo32, $repo64, $repoARM)) {
	foreach (@components) {
		my $curdir = cwd();
...
if ($dups[0] || $dups[1] || $dups[2]) {
	...
	foreach my $pkgname (sort keys %packages) {
		...
		for my $i (0 .. 2) {
			next unless $dups[$i];
			my $repotype = $i ? $repo64 : $repo32 : $repoARM;
...
if ($location || $notexist || $vercheck) {
	print "32/64 bit & ARM repo package location/existance report:\n";
	foreach (sort keys %packages) {
		unless (exists($packages{$_}{$repo32}[0]) && exists($packages{$_}{$repo64}[0]) && exists($packages{$_}{$repoARM}[0])) {
			if ($notexist) {
		...
		my $rARM_pkgname = join('', keys(%{${$packages{$_}{$repoARM}}[0]}));

		${$packages{$_}{$repo32}}[0]{$r32_pkgname}{comp} =~ /^$repo32(.*)$/;
		my $r32_comp = $1;
		${$packages{$_}{$repo64}}[0]{$r64_pkgname}{comp} =~ /^$repo64(.*)$/;
		my $r64_comp = $1;
		${$packages{$_}{$repoARM}}[0]{$rARM_pkgname}{comp} =~ /^$repoARM(.*)$/;
		my $rARM_comp = $1;

		if ($r32_comp ne $r64_comp ne $rARM_comp) {
			if ($location) {
			...
			if ($vercheck) {
				if ( (${$packages{$_}{$repo32}}[0]{$r32_pkgname}{ver} ne ${$packages{$_}{$repo64}}[0]{$r64_pkgname}{ver} ne ${$packages{$_}{$repoARM}}[0]{$rARM_pkgname}{ver}) || 
					(${$packages{$_}{$repo32}}[0]{$r32_pkgname}{rev} ne ${$packages{$_}{$repo64}}[0]{$r64_pkgname}{rev} ne ${$packages{$_}{$repoARM}}[0]{$rARM_pkgname}{rev}) ) {
...
sub FindSibsFor {
	...
	unless (defined($sibling_id)) {
		foreach my $filename (keys %{${$packages{$rpmname}{$repoARM}}[0]}) {
			$sibling_id = ${$packages{$rpmname}{$repoARM}}[0]{$filename}{sibling};
		}
	}
	unless (defined($sibling_id)) {
	...
	foreach my $pkgname (keys %packages) {
		for my $repotype (($repo32, $repo64, $repoARM)) {
			foreach my $entry (@{$packages{$pkgname}{$repotype}}) {
				foreach my $filename (keys %{$entry}) {
