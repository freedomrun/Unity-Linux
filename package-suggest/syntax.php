	    ...
	    $id  = 'bureaucracy'.$opt['idx'];
            ...
            switch($opt['cmd']){
                ...
                case 'submit':
                    ...
                    break;
                case 'password':
                    $form->addElement(form_makePasswordField($name,$opt['label'],$id,$class));
                    break;
                case 'textbox':
                case 'number':
                case 'email':
                    $form->addElement(form_makeTextField($name,$value,$opt['label'],$id,$class));
                    break;
                case 'onoff':
                case 'yesno':
                    $chk = ($value) ? 'checked="checked"' : '';
                    $form->addElement('<label class="'.$class.'"><span>'.hsc($opt['label']).'</span>'.
                                      '<input type="checkbox" id="'.$id.'" name="'.$name.'" value="Yes" '.$chk.' /></label>');
                    break;
                case 'select':
                    ...
                    array_unshift($vals,' ');
                    $form->addElement(form_makeListboxField($name,$vals,$value,$opt['label'],$id,$class));
                    break;
                case 'static':
                ...
                case 'textarea':
                    ...
                    $form->addElement('<label class="'.$class.'"><span>'.hsc($opt['label']).'</span>'.
                                      '<textarea id= "'.$id.'" name="'.$name.'" rows="' . $rows . '" cols="10" class="edit">'.$value.'</textarea></label>');
                    break;
            }
        }
